...
func (u *ApplicationUsecase) DeleteEvent(ctx context.Context, eventID, applicationID, userID int64) error {
	if err := u.eventRepo.SoftDeleteEvent(ctx, eventID, applicationID, userID); err != nil {
		if errors.Is(err, entity.ErrEventNotFound) {
			return utils.ErrNotFound("event not found")
		}
		return utils.ErrInternal(err)
	}
	return nil
